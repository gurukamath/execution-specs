...
from typing import TYPE_CHECKING, Final, Tuple, assert_never, final
...
from ..exceptions import FrameCountError, InvalidFrameError
from ..fork_types import RegularGas, VersionedHash
...
def calculate_frame_transaction_intrinsic_cost(
    ...
    return IntrinsicGasCost(
        regular=RegularGas(
            base_regular_gas + tokens * GasCosts.TX_DATA_TOKEN_STANDARD
        ),
        calldata_floor=RegularGas(
            base_regular_gas + floor_tokens * GasCosts.TX_DATA_TOKEN_FLOOR
        ),
    )


def calculate_frame_transaction_gas_limit(
    tx: FrameTransaction, intrinsic_regular_gas: RegularGas
) -> Uint:
    """
    Calculate the total gas limit of a frame transaction.

    Frame transactions have no gas limit field. Their gas limit is
    derived instead: the sum of the transaction's intrinsic regular gas
    cost, as returned by `calculate_frame_transaction_intrinsic_cost`,
    ...
    return Uint(intrinsic_regular_gas) + total_frame_gas
