...
from .eoa_delegation import get_delegated_code_address, set_delegation
...
class TransactionOutput:
    ...
    gas_left: Uint
    """Regular gas remaining after execution."""
...
def create_evm(
    ...
    if tx_env.is_create:
        ...
        code_address = tx_env.recipient

        if tx_env.value > U256(0) and not is_account_alive(
            tx_env.state, current_target
        ):
            charge_state_gas_from_meter(gas_meter, StateGasCosts.NEW_ACCOUNT)

        recipient_code = get_code(
            tx_env.state,
            get_account(tx_env.state, current_target).code_hash,
        )
