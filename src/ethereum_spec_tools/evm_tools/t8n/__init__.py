...
class T8N(Load):
    ...
    def run(self) -> "TransitionToolOutput":
        ...
        ``Alloc`` directly (the field is typed ``LazyAlloc | Alloc``;
        EELS hands over the already-materialized form), the ``Result``
        (state root, receipts, rejected txs, block exception, …), and
        the encoded transaction body as raw RLP bytes. The JSON CLI
        surface lives in :func:`.cli.write_t8n_outputs`.
        """
